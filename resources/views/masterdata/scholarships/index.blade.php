    ...
    @section('content')
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
                <div>
                    <a href="{{ route('masterdata.scholarships.create') }}" class="inline-block">
                        <button type="button"
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($scholarships as $data)
                            @foreach ($data->scholarship_detail as $detail)
                                <tr
                                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td scope="row"
                                    ...
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        <a href="{{ route('masterdata.scholarships.edit', $detail->scholarship_detail_id) }}"
                                            class="font-medium text-yellow-200 dark:text-blue-500 hover:underline">Edit</a>
                                        <button
                                            onclick="confirmDelete('{{ route('masterdata.scholarships.destroy', $detail->scholarship_detail_id) }}')"
                                            type="button"
        ...
        <div id="deleteModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div id="alert-additional-content-2" class="p-4 mb-4 text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
                ...
                <div class="flex">
                    <form id="deleteForm" method="post" action="{{ route('masterdata.scholarships.destroy', $detail->scholarship_detail_id ?? '') }}">
                        @csrf
                        @method('DELETE')
