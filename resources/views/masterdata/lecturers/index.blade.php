<x-app-layout>
    ...
    @section('href_descendant_folder', route('masterdata.lecturers.index')) 
    @section('descendant_folder', '/ Lecturer')
    @section('content')
    <style>
        #success-message {
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
            <div>
                <a href="{{ route('masterdata.lecturers.create', 'null') }}" class="inline-block">
                    <button  type="button" class="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-500 dark:focus:ring-blue-800">Tambah Dosen Wali</button>
                </a>
            </div>
            ...
            <tbody>
                @if ($lecturers->isEmpty())
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td colspan="6" scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center">Tidak ada data Dosen</td>
                    </tr>
                    @else
                    @foreach ($lecturers as $data)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <button onclick="confirmDelete('{{ route('masterdata.lecturers.destroy', $data->lecturer_id) }}')" type="button" class="font-medium text-red-600 dark:text-blue-500 hover:underline">Hapus</button>
    ...
    <div id="deleteModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div id="alert-additional-content-2" class="p-4 mb-4 text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
            ...
            <div class="flex">
                <form id="deleteForm" method="post" action="{{ route('masterdata.lecturers.destroy', $data->lecturer_id ?? '') }}">
                    @csrf
                    @method('DELETE')
