    ...
    @section('content')
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
                <div>
                    <a href="{{ route('masterdata.student_resignations.create') }}" class="inline-block">
                        <button type="button"
                        ...
                        @foreach ($data->student_resignation_detail as $detail)
                            <tr
                                ...
                                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <a href="{{ route('masterdata.student_resignations.edit', $detail->student_resignation_detail_id) }}"
                                        class="font-medium text-yellow-200 dark:text-blue-500 hover:underline">Edit</a> <br>
                                    <button
                                        onclick="confirmDelete('{{ route('masterdata.student_resignations.destroy', $detail->student_resignation_detail_id) }}')"
                                        type="button"
        ...
        <div id="deleteModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div id="alert-additional-content-2" class="p-4 mb-4 text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
                ...
                <h3 class="text-lg font-medium">Hapus data?</h3>
                </div>
                <div class="mt-2 mb-4 text-sm">
                    Apakah Anda yakin ingin menghapus data ini? Tindakan ini tidak dapat diurungkan.
                </div>
                <div class="flex">
                    <form id="deleteForm" method="post" action="{{ route('masterdata.student_resignations.destroy', $detail->student_resignation_detail_id ?? '') }}">
                        @csrf
                        @method('DELETE')
