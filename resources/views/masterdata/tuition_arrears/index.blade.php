                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr
                            ...
                            <th scope="col" class="px-6 py-3 border-b">Jumlah Tunggakan</th>
                            <th scope="col" class="px-6 py-3 border-b">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($tuition as $data)
                            @foreach ($data->tuition_arrear_detail as $detail)
                                <tr
                                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td scope="row"
                                    ...
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        <a href="{{ route('masterdata.tuition_arrears.edit', $detail->tuition_arrear_details_id) }}"
                                            class="font-medium text-yellow-200 dark:text-blue-500 hover:underline">Edit</a>
                                        <button @if ($data->name == 'admin') hidden @endif
                                            onclick="confirmDelete('{{ route('masterdata.tuition_arrears.destroy', $detail->tuition_arrear_details_id) }}')"
                                            type="button"
        ...
        <div id="deleteModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div id="alert-additional-content-2"
                ...
                <div class="flex items-center">
                    ...
                    <h3 class="text-lg font-medium">Hapus tunggakan?</h3>
                </div>
                <div class="mt-2 mb-4 text-sm">
                    Apakah Anda yakin ingin menghapus tunggakan ini? Tindakan ini tidak dapat diurungkan.
                </div>
                <div class="flex">
                    <form id="deleteForm" method="post" action="{{ route('masterdata.tuition_arrears.destroy', $detail->tuition_arrear_details_id ?? '') }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
