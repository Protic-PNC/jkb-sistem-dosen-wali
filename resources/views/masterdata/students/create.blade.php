            ...
            <div class="max-w-7xl sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        ...
                        @php
                        $userId = $user;
                        $isNewStudent = !$user ? 1 : 0; // Jika user null, maka default ke form tambah mahasiswa
                        @endphp

                        <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Pilih atau Tambah Mahasiswa</h2>
                            <form action="{{ route('masterdata.students.store', $user ?? 'null') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                {{-- @if (!$isNewStudent)
                                    @method('PUT')
                                @endif --}}
                                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                                    ...
                                    <input type="hidden" name="is_new_student" id="is_new_student" value="{{ $isNewStudent }}">
                                    <input type="hidden" name="user_id" id="user_id" value="{{ $user->id ?? 'null' }}">


                                    <!-- Dropdown Mahasiswa dengan Pencarian -->
                                    <div id="select_student_container" @if($isNewStudent) style="display:none;" @else style="display:block;" @endif>
                                    ...
                                    <!-- Tambah Mahasiswa -->
                                    <div id="add-student-form" @if(!$isNewStudent) style="display:none;" @endif>
                                    ...
                                    <!-- Checkbox untuk menambah mahasiswa baru -->
                                    <div @if($isNewStudent) hidden @endif class="w-full">
                                        <label for="add_new_student" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                            <input type="checkbox" id="add_new_student" onchange="toggleAddStudentForm(this)" class="mr-2" @if($isNewStudent) checked @endif> Tambah Mahasiswa Baru
                                        </label>
                                    </div>
