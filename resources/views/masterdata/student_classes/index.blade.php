...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
       ...
       <form action="{{ route('masterdata.student_classes.destroy.selected') }}" method="post">
        @csrf
            ...
            <div id="floating-card" class="fixed bottom-4 right-4 p-4 duration-300 bg-white opacity-0 pointer-events-none">
                <button type="submit" id="deleteAllSelectedButton" class="text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-500 dark:focus:ring-red-800">
                ...
                <tbody>
                    @if ($studentClass)
                        @foreach ($studentClass as $data)
                            <tr id="class_ids{{ $data->class_id }}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <td class="w-4 p-4">
                                    <div class="flex items-center">
                                        <input name="ids[]" value="{{ $data->class_id }}" type="checkbox" class="checkbox-item w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                ...
                                <td class="px-6 py-4">
                                    <button onclick="confirmDelete('{{ route('masterdata.student_classes.destroy', $data->class_id) }}')" type="button" class="font-medium text-red-600 dark:text-blue-500 hover:underline">Hapus</button>
    ...
    <div id="deleteModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div id="alert-additional-content-2" class="p-4 mb-4 text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
            ...
            <div class="flex">
                <form id="deleteForm" method="post" action="{{ route('masterdata.student_classes.destroy', $data->class_id ?? '') }}">
                    @csrf
                    @method('DELETE')
